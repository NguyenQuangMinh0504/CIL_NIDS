...
def encode_numeric_min_max_scale(df: pd.DataFrame, name):
...
def check_invalid_data(df: pd.DataFrame) -> None:
    ...
    logging.info(df.isna().any(axis=1).sum())
    logging.info("Number of null value ...")
    logging.info(df.isnull().any(axis=1).sum())
